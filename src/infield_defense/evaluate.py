...
def print_table_from_csv(csv_path: Path) -> None:
    ...
    # Group by (method, n_robots, vmax, tau_handoff)
    keys = {}
    for r in rows:
        key = (r["method"], int(r["n_robots"]), float(r["vmax"]), float(r["tau_handoff"]))
        keys.setdefault(key, []).append(r)

    print("method,n,vmax,tau_handoff,success_pct,t_total_mean,t_total_std,t_total_med,t_total_p90")
    for (method, n, vmax, tau), group in sorted(keys.items()):
        ...
        print(f"{method},{n},{vmax},{tau},{success_pct:.1f},{mean:.3f},{std:.3f},{median:.3f},{p90:.3f}")
...
def main() -> None:
    ...
    run_sweep(out_csv=out, k_trials=50, seed=0, tau_handoff=0.0)
