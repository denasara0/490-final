...
\begin{itemize}
    ...
    \item a transparent post-capture delivery-time model (direct vs. relay),
...
\noindent The primary fielder is selected by
\begin{equation}
i^\ast = \arg\min_i \tau_i^\ast .
\end{equation}

\subsection{Delivery model: direct vs. relay}
After interception, we compare direct carry to a one-hop relay using straight-line legs at speed \(v_{\max}\). Let \(p_{\mathrm{base}}\) be the base position and let \(\tau_{\mathrm{hp}}\ge 0\) be a fixed handoff delay:
\begin{equation}
...
\begin{itemize}
    ...
    \item \texttt{coordination.select\_primary\_fielder\_predicted} (argmin over bids).
\end{itemize}
